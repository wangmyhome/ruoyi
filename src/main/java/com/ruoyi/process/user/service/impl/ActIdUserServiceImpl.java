...
public class ActIdUserServiceImpl implements IActIdUserService
{
    ...
    public List<SysUser> selectUnAssociatedSystemUserList(SysUser sysUser) {
        ...
        userList.forEach(user -> {
            ...
            sysUserMap.put(user.getUserName(), user); //原user.getLoginName()
        });
        idUserList.forEach(idUser -> {
            SysUser tempUser = userMapper.selectUserByUserName(idUser.getId());
            if (tempUser != null) {
                userList2.add(tempUser);
            }
        });
        userList2.forEach(user -> {
            if (sysUserMap.containsKey(user.getUserName())) {  //原user.getLoginName()
                sysUserMap.remove(user.getUserName()); //原user.getLoginName()
            }
        });
